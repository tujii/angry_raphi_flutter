...
void main() {
  group('Bronze Badge Display Logic Tests', () {
    ...
    test('should calculate correct ranks with tied positions', () {
      ...
      expect(RankingUtils.calculateRank(testUsers, 0), equals(1)); // S.C. - Gold
      expect(RankingUtils.calculateRank(testUsers, 1), equals(2)); // M.J. - Silver  
      expect(RankingUtils.calculateRank(testUsers, 2), equals(2)); // J.D. - Silver (tied)
      expect(RankingUtils.calculateRank(testUsers, 3), equals(4)); // I.G. - Rank 4 (should NOT show bronze)
      expect(RankingUtils.calculateRank(testUsers, 4), equals(5)); // R.U. - Rank 5
    });
    ...
    test('bronze should appear with different user distribution', () {
      ...
      final differentUsers = [
        User(id: '1', initials: 'A.B.', raphconCount: 10, createdAt: DateTime.now()),
        User(id: '2', initials: 'C.D.', raphconCount: 8, createdAt: DateTime.now()),  
        User(id: '3', initials: 'E.F.', raphconCount: 6, createdAt: DateTime.now()),
        User(id: '4', initials: 'G.H.', raphconCount: 4, createdAt: DateTime.now()),
      ];
      
      expect(RankingUtils.calculateRank(differentUsers, 0), equals(1)); // Gold
      expect(RankingUtils.calculateRank(differentUsers, 1), equals(2)); // Silver  
      expect(RankingUtils.calculateRank(differentUsers, 2), equals(3)); // Bronze - THIS would show!
      expect(RankingUtils.calculateRank(differentUsers, 3), equals(4)); // No badge
    });
  });
}