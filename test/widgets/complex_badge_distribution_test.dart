  ...
  group('Complex Badge Distribution Test', () {
    ...
    setUp(() {
      ...
      testUsers = [
        ...
        User(id: '1', initials: 'G1', raphconCount: 15, createdAt: DateTime.now()),
        User(id: '2', initials: 'G2', raphconCount: 15, createdAt: DateTime.now()),
        
        // 4 Silver users (10 Raphcons each)
        User(id: '3', initials: 'S1', raphconCount: 10, createdAt: DateTime.now()),
        User(id: '4', initials: 'S2', raphconCount: 10, createdAt: DateTime.now()),
        User(id: '5', initials: 'S3', raphconCount: 10, createdAt: DateTime.now()),
        User(id: '6', initials: 'S4', raphconCount: 10, createdAt: DateTime.now()),
        
        // 10 Bronze users (7 Raphcons each)
        User(id: '7', initials: 'B1', raphconCount: 7, createdAt: DateTime.now()),
        User(id: '8', initials: 'B2', raphconCount: 7, createdAt: DateTime.now()),
        User(id: '9', initials: 'B3', raphconCount: 7, createdAt: DateTime.now()),
        User(id: '10', initials: 'B4', raphconCount: 7, createdAt: DateTime.now()),
        User(id: '11', initials: 'B5', raphconCount: 7, createdAt: DateTime.now()),
        User(id: '12', initials: 'B6', raphconCount: 7, createdAt: DateTime.now()),
        User(id: '13', initials: 'B7', raphconCount: 7, createdAt: DateTime.now()),
        User(id: '14', initials: 'B8', raphconCount: 7, createdAt: DateTime.now()),
        User(id: '15', initials: 'B9', raphconCount: 7, createdAt: DateTime.now()),
        User(id: '16', initials: 'B10', raphconCount: 7, createdAt: DateTime.now()),
        
        // 5 users without badges (5 Raphcons each)
        User(id: '17', initials: 'N1', raphconCount: 5, createdAt: DateTime.now()),
        User(id: '18', initials: 'N2', raphconCount: 5, createdAt: DateTime.now()),
        User(id: '19', initials: 'N3', raphconCount: 5, createdAt: DateTime.now()),
        User(id: '20', initials: 'N4', raphconCount: 5, createdAt: DateTime.now()),
        User(id: '21', initials: 'N5', raphconCount: 5, createdAt: DateTime.now()),
      ];
    });
    ...
    test('should have correct unique score distribution', () {
      final uniqueScores = testUsers
          .map((u) => u.raphconCount)
          .toSet()
          .toList()
        ..sort((a, b) => b.compareTo(a));
    ...
    test('should distribute badges correctly: 2 Gold, 4 Silver, 10 Bronze', () {
      ...
      for (int i = 0; i < testUsers.length; i++) {
        ...
        if (hasBadge) {
          ...
          print('${testUsers[i].initials} (${testUsers[i].raphconCount}) → $badgeText');
        } else {
          noBadgeCount++;
          print('${testUsers[i].initials} (${testUsers[i].raphconCount}) → NO BADGE');
        }
      }

      // Verify the exact distribution
      expect(goldCount, equals(2), reason: 'Should have exactly 2 Gold badges');
      expect(silverCount, equals(4), reason: 'Should have exactly 4 Silver badges');
      expect(bronzeCount, equals(10), reason: 'Should have exactly 10 Bronze badges');
      expect(noBadgeCount, equals(5), reason: 'Should have exactly 5 users without badges');
    ...
    test('should handle large tie scenarios correctly', () {
      ...
      expect(usersByScore[15]!.length, equals(2), reason: '2 users with 15 Raphcons');
      expect(usersByScore[10]!.length, equals(4), reason: '4 users with 10 Raphcons');  
      expect(usersByScore[7]!.length, equals(10), reason: '10 users with 7 Raphcons');
      expect(usersByScore[5]!.length, equals(5), reason: '5 users with 5 Raphcons');
      ...
      for (final user in goldUsers) {
        ...
        expect(getBadgeText(getBadgePosition(testUsers, index)), equals('GOLD'));
      }
      
      // Verify all silver users get silver
      for (final user in silverUsers) {
        final index = testUsers.indexOf(user);
        expect(getBadgeText(getBadgePosition(testUsers, index)), equals('SILVER'));
      }
      
      // Verify all bronze users get bronze
      for (final user in bronzeUsers) {
        final index = testUsers.indexOf(user);
        expect(getBadgeText(getBadgePosition(testUsers, index)), equals('BRONZE'));
      }
